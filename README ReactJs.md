...
export const RHFYouTubeForm = () => {
  const form = useForm<FormValues>({
    ...
    mode: "onTouched",  // mode controls when validation runs for your form fields. it can be onChange, onBlur, onSubmit, all
  });
  ...
  const handleGetValues = () => {
    console.log("Get values", getValues("username")); // get the value of input having name "username"
  };

  const handleSetValue = () => {
    setValue("username", "", {  // set the value in the input naming the "username"
      shouldValidate: true,
      shouldDirty: true,
      shouldTouch: true,
    });
  ...
  useEffect(() => {
    if (isSubmitSuccessful) {
      reset(); // Reset is basically resetting the values to initial, if any value is passed then that value
    }
  ...
  return (
    <div>
      ...
      {/* second argument of handleSubmit takes the error function, means when error comes then this will run */}
      <form onSubmit={handleSubmit(onSubmit, onError)}>
        ...
        <div className="form-control">
          ...
          <input
            ...
            id="age"
            {...register("age", {
              valueAsNumber: true, // input gives default as string, but convert it as number
        ...
        {/* Here Trigger is use the run the schema validation (specific for channel input) on click */}
